 ...
 *
 * Зародыш команды `agentmeter verify` (1.3). Критерий — тот, которым 1.3 был
 * закрыт: не «всё до токена» (так не будет никогда, хвостовые прогревы следа в
 * логах не оставляют), а измеренная поправка. Порог и пол числа точных сессий
 * ниже — это тревога на регресс модели, а не украшение.
 */
/** Ни одна сессия не расходится по чтению кэша сильнее этого. */
const MAX_DRIFT = 0.035
/**
 * Сколько сессий обязано сходиться **до токена**. Замерено при закрытии 1.3:
 * 5 из 16. Стало меньше — сломана модель, а не тест; порог не трогать, идти
 * искать. Файлы на диске Claude Code подчищает сам, поэтому знаменатель плавает,
 * а числитель — нет.
 */
const MIN_EXACT = 5
...
const ok = worst <= MAX_DRIFT && exact >= MIN_EXACT
if (!ok) {
  console.error(
    `\n✗ расхождение ${(worst * 100).toFixed(2)}% при пороге ${(MAX_DRIFT * 100).toFixed(1)}%, ` +
      `точных ${exact} при минимуме ${MIN_EXACT} — это регресс модели, а не теста`,
  )
}
process.exit(ok ? 0 : 1)
