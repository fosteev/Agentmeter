...
const SPEC_LINES: Record<string, Array<[number, number]>> = {
  ...
  // Живой подписи (6.1) в макете нет — как нет темпа (2.3) и указателя
  // контекста (2.6). Блоков у неё два, и оба свои по существу: строка ленты
  // (619–631) даёт поля колонки, в которой подпись стоит, а вторая строка
  // строки агента в попапе (351–367) — это ровно та же живая подпись в своём
  // первом контексте, с той же точкой состояния и теми же числами. Довод — в
  // `design-implementation.md`, рядом с таким же заимствованием у
  // `TaskSubagents`.
  'TaskLive.tsx': [
    [619, 631],
    [351, 367],
  ...
  // под ней. Строка 1739 («Показать эти 24 паузы в ленте») в компоненте не
  ...
  // данными. Числа взяты у пустого экрана развёртки (927–931), потому что это
  ...
  'HistoryBars.tsx': [[1305, 1340]],
  'HistoryHeatmap.tsx': [[1345, 1395]],
  'HistorySide.tsx': [[1398, 1450]],
}
