...
const SPEC_LINES: Record<string, Array<[number, number]>> = {
  ...
  // Табы провайдеров над списком лимитов (7.1) — свой блок макета целиком:
  // отчёркнутый ряд, поля подписи и квадратная точка уровня.
  'PopupLimitTabs.tsx': [[416, 429]],
  ...
  // «Что показывать в попапе» (7.5) стоит на месте карточек потолков и
  // свёрстано их числами: карточка та же и в той же колонке. Тумблеры внутри —
  // из блока переключателей (1236–1243), как везде, где стоит `Switch`.
  'SettingsPopupLimits.tsx': [
    [1197, 1218],
    [1236, 1243],
  ],
  // «Проценты лимита от провайдера» (6.3, 6.4) — блок того же раздела, и
  // свёрстан он теми же числами: свой блок в макете у него не нарисован, макет
  // старше находки. Довод — в `design-implementation.md`.
  'SettingsUsage.tsx': [
    [1197, 1218],
    [1236, 1243],
  ],
...
const OFF_SPEC: Record<string, number[]> = {
  'LimitBar.tsx': [3],
  // Ноль — сброс браузерного отступа у системной кнопки обновления (7.2), а не
  // число из макета: в макете на её месте нарисован `div`, у которого своего
  // отступа нет вовсе, а `button` без сброса разъезжается на 22 точки бокса.
  // Тот же довод, что у `input` ниже, и та же цена отказа: красить системный
  // элемент своими прямоугольниками значит потерять клавиатуру.
  'PopupHeader.tsx': [0],
