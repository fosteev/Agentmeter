...
describe('схема индекса', () => {
  ...
  it('отказывается открывать базу от более новой версии', () => {
    ...
    // Ловит дескриптор, забытый открытым при отказе. На Windows он держит
    // файл: совет из самого сообщения — «удалите индекс» — становится
    // невыполнимым, а на POSIX это видно только тем, что файл не удаляется в
    // уборке теста (EBUSY на живом раннере).
