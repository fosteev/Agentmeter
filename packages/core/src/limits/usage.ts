 ...
 *
 * **С 7.5 приложение этим путём не ходит:** хук снят, журнал наполняет ответ
 * провайдера (6.3). Разбор остался вместе с эталоном — он и есть запись
 * находки 21 (процент лимита Claude отдаёт только строка состояния), и живые
 * журналы, собранные до 7.5, разобраны им же.
 */
