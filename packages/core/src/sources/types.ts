...
export interface PrefixBlock {
  ...
  items: number
  /**
   * Как эти штуки зовут — в порядке лога (4.9). Скиллы, сабагенты, имена
   * отложенных тулов, пути файлов памяти.
   *
   * **Поля нет, когда источник имён не назвал**, и пустой массив здесь значил бы
   * другое: «штук нет». У Codex память приезжает безымянными `input_text`, у
   * остатка перечислимых штук не существует вовсе — обоим случаям на экране
   * положена своя фраза, а не пустой список.
   *
   * Имя — это **идентичность**, а не подпись: у файла памяти здесь абсолютный
   * путь, хотя показывается он сокращённым. `displayPath` из лога относителен
   * рабочему каталогу, и один и тот же `CLAUDE.md` записан в разных сессиях как
   * `Pilot7/CLAUDE.md`, `../CLAUDE.md` и `../../Pilot7/CLAUDE.md` — склейка по
   * нему превратила бы один файл в три.
   */
  names?: string[]
}
