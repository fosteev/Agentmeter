...
export const ru = {
  ...
  settings: {
    ...
    megabytes: '{{value}} МБ',
    caps: 'Потолки лимитов',
    claudeCaps: 'Claude — потолки',
    codexCaps: 'Codex — потолки',
    capsWeight: 'вес cache_read',
    capsNotMeasured: 'не измерено',
    // Потолок Claude не выбирается кнопкой: объявленных тарифов в этих
    // единицах не существует, а процент от выдуманного знаменателя — то самое
    // враньё, которого продукт себе не позволяет (7.4).
    capsMeasuredNote: 'измеряется по ответам провайдера — ниже',
    capsCodexNote: 'потолка нет: провайдер сообщает процент готовым',
    capsCodexStale: 'в логе он написан в момент запроса — свежесть чинит запрос ниже',
